...
import { ArrowLeft, Check, Shield, Clock, CreditCard, QrCode, Flag, Loader2 } from "lucide-react"
...
export default function PagamentoClient() {
  ...
  useEffect(() => {
    const loadData = async () => {
      ...
      if (!user) {
        router.push("/auth/login?redirect=/pagamento" + (courseId ? `?courseId=${courseId}` : ""))
        return
      }
      ...
      if (courseId) {
        ...
        // Load first active course if no courseId provided
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      // Redirecionar para o checkout do InfinityPay
      if (data.url) {
      ...
      <header className="border-b border-border/40 bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 sticky top-0 z-50">
        <div className="container mx-auto px-4 py-4 flex items-center justify-between">
          ...
          <Link href="/" className="flex items-center gap-2">
            ...
            <span className="font-display text-xl font-bold tracking-tight">APEXX</span>
          </Link>
          <div className="w-20" />
        </div>
      </header>
          ...
          <div className="grid lg:grid-cols-5 gap-8">
            ...
            <div className="lg:col-span-3">
              <Card className="border-border/50">
                <CardHeader>
                  <CardTitle className="text-xl">Metodo de Pagamento</CardTitle>
                  <CardDescription>Pague com PIX ou Cartao de Credito</CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="p-4 rounded-xl bg-card border border-border/50 text-center">
                      ...
                      <p className="text-xs text-muted-foreground">Credito ou debito</p>
                    </div>
                  </div>
              ...
              <Card className="border-primary/30 sticky top-24">
                ...
                <CardContent className="space-y-6">
                  ...
                  <div className="flex gap-4">
                    ...
                    <div>
                      <h3 className="font-semibold text-foreground">{course?.title || "Curso"}</h3>
                      <p className="text-sm text-muted-foreground">Acesso vitalicio</p>
                    </div>
                  </div>
